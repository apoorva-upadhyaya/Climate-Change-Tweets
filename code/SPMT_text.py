...
def create_resample(train_sequence,train_enc,train_enc_senti):
    df = pd.DataFrame(list(zip(train_sequence,train_enc,train_enc_senti)), columns =['text','pol','sent'],index=None)
    blv=(df[df['pol'] == 0])
...
for train_index,test_index in kf.split(total_sequence,total_labels_stance_enc):
    ...
    model=Model([input1_stance,input1_senti],[stance_specific_output,senti_specific_output,stance_shared_output,senti_shared_output])
    ...
    # oversample
    train_sequence,train_enc,train_enc_senti=create_resample(train_sequence,train_enc,train_enc_senti)
    ...
    model.fit([train_sequence,train_sequence],[train_stance,train_senti,train_stance,train_senti], shuffle=True,validation_split=0.2,epochs=20,verbose=2)
    predicted = model.predict([test_sequence,test_sequence])
