...
def create_resample(train_sequence,train_sequence_topic,train_enc):
    df = pd.DataFrame(list(zip(train_sequence,train_sequence_topic,train_enc)), columns =['text','topic','pol'],index=None)
    blv=(df[df['pol'] == 0])
    ...
    return train_sequence,train_sequence_topic,train_enc
...
for train_index,test_index in kf.split(total_sequence,total_labels_stance_enc):
    ...
    lstm = Bidirectional(LSTM(100, name='lstm_inp2', activation='tanh',dropout=.2,kernel_regularizer=regularizers.l2(0.07)))(input_topic)
    topic_final = Dense(100, activation="relu")(lstm)
    ...
    IRAScores=Lambda(attentionScoresIRA)([Q_t,K_t,V_t,Q_e,K_e,V_e])
    ...
    model=Model([input1,input2],[stance_specific_output])
    ...
    train_sequence,train_sequence_topic,train_enc=create_resample(train_sequence,train_sequence_topic,train_enc)
    ...
    model.fit([train_sequence,train_sequence_topic],[train_stance], shuffle=True,validation_split=0.2,epochs=20,verbose=2)
